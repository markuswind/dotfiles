...
function! wind#statusline#fenc() abort
...
function! wind#statusline#rhs() abort
  let l:rhs=' '
  if winwidth(0) > 80
...
let s:wind_statusline_status_highlight=s:default_lhs_color
let s:async=0

function! wind#statusline#async_start() abort
  let s:async=1
  call wind#statusline#check_modified()
endfunction

function! wind#statusline#async_finish() abort
  let s:async=0
  call wind#statusline#check_modified()
endfunction

function! wind#statusline#check_modified() abort
  if &modified && s:wind_statusline_status_highlight != s:modified_lhs_color
    let s:wind_statusline_status_highlight=s:modified_lhs_color
    call wind#statusline#update_highlight()
  elseif !&modified
    if s:async && s:wind_statusline_status_highlight != s:async_lhs_color
      let s:wind_statusline_status_highlight=s:async_lhs_color
      call wind#statusline#update_highlight()
    elseif !s:async && s:wind_statusline_status_highlight != s:default_lhs_color
      let s:wind_statusline_status_highlight=s:default_lhs_color
      call wind#statusline#update_highlight()
    endif
  endif
...
function! wind#statusline#update_highlight() abort
